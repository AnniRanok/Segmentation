...
def load_checkpoint(filepath, model, optimizer=None):
    """Loads the state of the model and optimizer from a checkpoint."""
    checkpoint = torch.load(filepath)
    model.load_state_dict(checkpoint['model_state_dict'])
    if optimizer:
        optimizer.load_state_dict(checkpoint['optimizer_state_dict'])
    return checkpoint.get('epoch'), checkpoint.get('loss')
...
def find_best_checkpoint(model, optimizer=None, directory=Config.MODEL_SAVE_DIR):
    ...
    for filename in os.listdir(directory):
        if filename.endswith(".pth"):
            ...
            _, loss = load_checkpoint(checkpoint_path, model, optimizer)
